...
$.validator.addMethod("dateGreaterThanToday",
	function(value) {
		if (!/Invalid|NaN/.test(new Date(value))) {
			...
			return new Date(value + ' 00:00:00') > today;
		}

		return isNaN(value) || (Number(value) < Number());
...
$.validator.addMethod('phoneNumbers', function(phone_number, element) {
	phone_number = phone_number.replace(/\s+/g, '');
	return this.optional(element) || phone_number.match(/^01([0~1]|[6~9])-\d{3,4}-\d{4}$/);
}, '잘못된 휴대폰 번호입니다. 숫자, – 를 포함한 숫자만 입력하세요.');

$.validator.addMethod('prodCode', function(prodCode, element) {
	return this.optional(element) || prodCode.match(/^[a-zA-Z#]{2}(\d{12})$/);
}, '잘못된 상품 코드 입니다.');